...
validate:
	@for f in .claude-plugin/marketplace.json templates/project-settings.json plugins/*/.claude-plugin/plugin.json plugins/*/.codex-plugin/plugin.json; do jq -e . "$$f" >/dev/null && echo "OK  $$f" || { echo "BAD $$f"; exit 1; }; done
