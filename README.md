...
- Deps and tests: `make deps` (jq + bats-core + Python, with PyYAML in `.venv`), `make validate` (jq-check every manifest and the template), `make test` (all bats, including real YAML frontmatter parsing). Python/PyYAML are development-only dependencies; installed plugins do not need them. To use an existing Python environment with PyYAML, run `make test PYTHON=/path/to/python`.
