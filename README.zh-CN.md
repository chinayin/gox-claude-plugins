...
| Claude 执行含 `git … push` 的 Bash 命令 | 扫 `HEAD` 上不在任何远端的提交（`--all` / `--mirror` 时扩到所有本地分支）。没有待推送提交：不扫直接放行。干净：静默放行。有发现：**拒绝**这次 push，给模型一行一条的发现列表（规则、文件:行、提交、fingerprint，最多 10 条）、两句裁定规则，以及完整协议的路径。 |
| 扫描器（或 `jq`）缺失、扫描器出错 | 拦下并说明原因，而不是静默放行。会静默放行的闸门等于没有。 |

拦截文案刻意压短（单条发现约 500 字符），因为它会进主 agent 的上下文。完整协议放在 [`plugins/gox-guard/TRIAGE.md`](plugins/gox-guard/TRIAGE.md)，模型需要时才读：

1. 先判断：真密钥还是误报。
2. 真密钥：从对应提交里删掉（如 `git reset --soft`、改文件、重新提交），并告诉你这个值已经进过本地历史、建议轮换。绝不把真密钥加进白名单。
3. 单行误报：在该行加 `# betterleaks:allow` 注释。
4. 已提交、不值得重写的误报，或成规律的误报（测试夹具、示例）：把 fingerprint 追加到 `.betterleaksignore`，或在 `.betterleaks.toml` 里加路径/正则白名单。两者都是普通的提交文件，会出现在 review 里。
